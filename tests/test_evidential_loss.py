...
def test_a_non_contiguous_column_set_maps_targets_correctly() -> None:
    """The charged column set need not be a contiguous range.

    Local positions have to come from a scatter rather than subtracting an
    offset -- otherwise every label lands on the wrong row and the loss silently
    optimises the wrong thing. Main no longer has a global noise column (which is
    what used to make the set non-contiguous), so inject one directly.
    """
    torch.manual_seed(0)
    net = WoeSiNet(1024, 6, 2, _woe_args())
    features = torch.randn(4, net.feature_dim)
    net._current_task_class_indices = lambda t, device: torch.tensor(
        [0, 3, 4, 5], dtype=torch.long, device=device
    )
